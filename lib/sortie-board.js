...
/** クライアントの削除・更新意図が after に残っているか */
function clientIntentPreserved(after, client) {
  const afterSorties = Array.isArray(after?.sorties) ? after.sorties : [];
  const afterById = new Map(afterSorties.map((s) => [String(s.id), s]));
  const afterDeleted =
    after?.deletedSorties && typeof after.deletedSorties === 'object' ? after.deletedSorties : {};
  const clientDeleted =
    client?.deletedSorties && typeof client.deletedSorties === 'object' ? client.deletedSorties : {};

  for (const [id, at] of Object.entries(clientDeleted)) {
    const delAt = Number(at) || 0;
    const s = afterById.get(String(id));
    if (!s) continue;
    // 削除より新しい更新で復活しているなら許容
    if (entityTs(s) > delAt) continue;
    // まだ残っている＝削除が落ちた
    return false;
  }

  for (const s of Array.isArray(client?.sorties) ? client.sorties : []) {
    const id = String(s?.id || '');
    if (!id) continue;
    const delAt = Number(afterDeleted[id] || clientDeleted[id] || 0);
    if (delAt && delAt >= entityTs(s)) continue;
    const cur = afterById.get(id);
    if (!cur) return false;
    if (entityTs(cur) < entityTs(s)) return false;
  }
  return true;
}

/** 同時 PUT でも削除が潰れないよう直列化＋リトライ */
let mergeWriteChain = Promise.resolve();

/** 現在の共有とクライアント送信分をマージして保存 */
export async function saveMergedBoard(clientBoard) {
  const run = async () => {
    let incoming = clientBoard;
    let last = null;
    for (let attempt = 1; attempt <= 6; attempt++) {
      const current = await loadBoard();
      const merged = mergeBoards(current, incoming, { now: Date.now() });
      last = await saveBoard(merged);
      // 書き込み直後に再読込し、他リクエストに潰されていないか確認
      const after = await loadBoard();
      if (clientIntentPreserved(after, incoming)) return after;
      // 競合時は最新の上に、自分の削除墓石・出撃をもう一度載せる
      incoming = {
        sorties: Array.isArray(incoming?.sorties) ? incoming.sorties : [],
        members: Array.isArray(incoming?.members) ? incoming.members : [],
        deletedSorties: {
          ...(after.deletedSorties || {}),
          ...(incoming?.deletedSorties || {}),
        },
      };
    }
    return last || sanitizeBoard(incoming);
  };
  const done = mergeWriteChain.then(run, run);
  mergeWriteChain = done.then(
    () => undefined,
    () => undefined
  );
  return done;
